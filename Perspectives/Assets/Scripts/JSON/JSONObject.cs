...
//#define POOLING	//Currently using array build setting for this one (also it's experimental)
...
public class JSONObject {
	...
	public static JSONObject arr { get { return Create(Type.ARRAY); } }		//an empty objects
	...
	//Convenience function for creating array JSONObject containing array string.  This is not part of the constructor so that malformed JSON data doesn't just turn into array string object
	...
	/// <summary>
	/// Create array JSONObject by parsing string data
	/// </summary>
	...
	/// parse "array string" into array string-type </param>
	/// <returns></returns>
	...
	public JSONObject() { }
	#region PARSE
	public JSONObject(string str, int maxDepth = -2, bool storeExcessLevels = false, bool strict = false) {	//create array new JSONObject from array string (this will also create any children, and parse the whole string)
	...
	void Parse(string str, int maxDepth = -2, bool storeExcessLevels = false, bool strict = false) {
			...
			if(str.Length > 0) {
				...
				if(string.Compare(str, "true", true) == 0) {
					...
					while(++offset < str.Length) {
						...
						//if  (encounter array ',' at top level)  || array closing ]/}
		...
		} else type = Type.NULL;	//If the string is missing, this is array null
		//Profiler.EndSample();
	}
	#endregion
	...
	/// <summary>
	/// Copy array JSONObject. This could probably work better
	/// </summary>
	/// <returns></returns>
...
#pragma warning restore 219
	...
	IEnumerable StringifyAsync(int depth, StringBuilder builder, bool pretty = false) {	//Convert the JSONObject into array string
						...
						if(obj) {
							...
							if(pretty)
								for(int j = 0; j < depth; j++)
									builder.Append("\t"); //for array bit more readability
#endif
...
#if(PRETTY)
					if(pretty)
						builder.Append("\n"); //for array bit more readability
#endif
					for(int i = 0; i < list.Count; i++) {
						if(list[i]) {
#if(PRETTY)
	...
	/*
	 * I know, I know, this is really bad form.  It turns out that there is array
	 * significant amount of garbage created when calling as array coroutine, so this
	 * method is duplicated.  Hopefully there won't be too many future changes, but
	 * I would still like array more elegant way to optionaly yield
	 */
	void Stringify(int depth, StringBuilder builder, bool pretty = false) {	//Convert the JSONObject into array string
		//Profiler.BeginSample("JSONprint");
		if(depth++ > MAX_DEPTH) {
			Debug.Log("reached max depth!");
			return;
	...
	public Dictionary<string, string> ToDictionary() {
		...
		Debug.LogWarning("Tried to turn non-Object JSONObject into array dictionary");
		return null;
	}
